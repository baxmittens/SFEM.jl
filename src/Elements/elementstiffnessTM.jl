...
function ipRintTM(state, matpars, 𝐁, grad𝐍_temp, 𝐍_temp, nodalU, nodalT, nodalTm1, εpl, detJ, w, Δt)
	ΔTr = transpose(𝐍_temp)*nodalT
	σ = ipRint(state, matpars, 𝐁, nodalU, εpl, detJ, w, ΔTr)
	q = ipRintT(state, matpars, grad𝐍_temp, 𝐍_temp, nodalT, nodalTm1, detJ, w, Δt)
	return vcat(σ,q)
end

@generated function elStiffnessTM(::Type{Val{NIPs}}, ::Type{Val{NNODES1}}, ::Type{Val{NNODES2}}, ::Type{Val{DIM}}, state, matpars, 𝐁s, 𝐍s_temp, grad𝐍s_temp, nodalU, nodalT, nodalTm1, εpls, detJs, wips, Δt) where {NIPs,NNODES1,NNODES2,DIM}
	DIMTimesNNODES = (DIM*NNODES1+NNODES2)
	DIMTimesNNODESSQ = DIMTimesNNODES*DIMTimesNNODES
	body = Expr(:block)
	for ip in 1:NIPs
		push!(body.args, quote
			Rel += ipRintTM(state[$ip], matpars, 𝐁s[$ip], grad𝐍s_temp[$ip], 𝐍s_temp[$ip], nodalU, nodalT, nodalTm1, εpls[$ip], detJs[$ip], wips[$ip], Δt)
...
function elStiffnessTM(el1::Tri{DIM, NNODES1, NIPs, DIMtimesNNodes1, Mat}, el2::Tri{DIM, NNODES2, NIPs, DIMtimesNNodes2}, dofmap1, dofmap2, U, Uprev, shapeFuns1, shapeFuns2, actt, Δt) where {DIM, NNODES1, NNODES2, NIPs, DIMtimesNNodes1, DIMtimesNNodes2, Mat}
	...
	return elStiffnessTM(Val{NIPs}, Val{NNODES1}, Val{NNODES2}, Val{DIM}, el1.state.state, el1.matpars, 𝐁s, 𝐍sT, grad𝐍sT, nodalU, nodalT, nodalTm1, εpls, detJs, wips, Δt)
end
