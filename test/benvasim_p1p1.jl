...
els1 = ElType[Tri3(SMatrix{2,nnodes_element,Float64,2*nnodes_element}(nodes[elinds,1:2]'), SVector{nnodes_element,Int}(elinds), state, matparsdict[matids[i]], Val{nips}) for (i,(elinds,state)) in enumerate(zip(connectivity, states))];
els2 = ElType[Tri3(SMatrix{2,nnodes_element,Float64,2*nnodes_element}(nodes[elinds,1:2]'), SVector{nnodes_element,Int}(elinds), state, matparsdict[matids[i]], Val{nips}) for (i,(elinds,state)) in enumerate(zip(connectivity, states))];
###
