...
meshfilepath3 = "../models/2d/beam_grob_tri3.msh"
meshfilepath6 = "../models/2d/beam_grob_tri6.msh"
mesh3 = GmshMesh(meshfilepath3);
mesh6 = GmshMesh(meshfilepath6);
nips = 7
...
states = [ElementStateVars2D(Val{nips},Val{nts}) for elinds in mesh6.connectivity];

matpars = MatPars(7000.0, 450.0, 1e-5, 1e-5, 0.0, 50.0, 50.0, 0.0, 2.1e11, 0.3, 200.0)
els1 = Tri{2,6,nips,12}[Tri6(SMatrix{2,6,Float64,12}(mesh6.nodes[elinds,1:2]'), SVector{6,Int}(elinds), state, matpars, Val{nips}) for (elinds,state) in zip(mesh6.connectivity, states)];
els2 = Tri{2,3,nips,6}[Tri3(SMatrix{2,3,Float64,6}(mesh3.nodes[elinds,1:2]'), SVector{3,Int}(elinds), state, matpars, Val{nips}) for (elinds,state) in zip(mesh3.connectivity, states)];

ndofs1 = size(mesh6.nodes,1)*2
dofmap1 = convert(Matrix{Int}, reshape(1:ndofs1,2,:))
ndofs2 = size(mesh3.nodes,1)*1
dofmap2 = convert(Matrix{Int}, reshape(ndofs1+1:ndofs1+ndofs2,1,:))
linelasticity = ProcessDomain(LinearElasticity, mesh6.nodes, mesh6.connectivity, els1, dofmap1, nips, nts, Val{2})
heatconduction = ProcessDomain(HeatConduction, mesh3.nodes, mesh3.connectivity, els2, dofmap2, nips, nts, Val{1})
