...
els1 = ElType1[Tri6(SMatrix{2,nnodes_element1,Float64,2*nnodes_element1}(nodes1[elinds,1:2]'), SVector{nnodes_element1,Int}(elinds), state, matparsdict[matids1[i]], Val{nips}) for (i,(elinds,state)) in enumerate(zip(connectivity1, states))];
...
for el in els1
	for line in lines
		...
		if all(map(x->x ∈ neumann_inds, inds))
			push!(neumann_els_M, Line(SMatrix{2,3,Float64,6}(nodes1[inds,1:2]'), inds, Val{nips_neumann}))
		end
	end
end
...
#dofmap2 = convert(Matrix{Int}, reshape(1:ndofs2,1,:))
###
