...
meshfilepath = "../models/2d/beam_grob_tri3.msh"
...
els1 = Tri{2,3,nips,6}[Tri3(SMatrix{2,3,Float64,6}(mesh.nodes[elinds,1:2]'), SVector{3,Int}(elinds), state, matpars, Val{nips}) for (elinds,state) in zip(mesh.connectivity, states)];
els2 = Tri{2,3,nips,6}[Tri3(SMatrix{2,3,Float64,6}(mesh.nodes[elinds,1:2]'), SVector{3,Int}(elinds), state, matpars, Val{nips}) for (elinds,state) in zip(mesh.connectivity, states)];
