...
# Thermo
bodyforceT(x, matpars, actt, ts=ts) = actt > 1 ? 10.0 : 0.0
...
els1 = ElType[Tri6(SMatrix{2,nnodes_element,Float64,2*nnodes_element}(nodes[elinds,1:2]'), SVector{nnodes_element,Int}(elinds), state, matpars, Val{nips}) for (i,(elinds,state)) in enumerate(zip(connectivity, states))];
els2 = ElType[Tri6(SMatrix{2,nnodes_element,Float64,2*nnodes_element}(nodes[elinds,1:2]'), SVector{nnodes_element,Int}(elinds), state, matpars, Val{nips}) for (i,(elinds,state)) in enumerate(zip(connectivity, states))];
###
...
neumann_inds = findall(x->isapprox(x[1],1.0,atol=1e-5), eachrow(nodes))
