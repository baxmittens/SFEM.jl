...
### Load mesh
meshfilepath = "../models/2d/patchtest_tri10.msh"
...
els1 = ElType[Tri10(SMatrix{2,nnodes_element,Float64,2*nnodes_element}(nodes[elinds,1:2]'), SVector{nnodes_element,Int}(elinds), state, matpars, Val{nips}) for (i,(elinds,state)) in enumerate(zip(connectivity, states))];
els2 = ElType[Tri10(SMatrix{2,nnodes_element,Float64,2*nnodes_element}(nodes[elinds,1:2]'), SVector{nnodes_element,Int}(elinds), state, matpars, Val{nips}) for (i,(elinds,state)) in enumerate(zip(connectivity, states))];
###
